function [featHandles] = generateFeatures(A,Fs,sampleSize)
...
%Number of features considered now = 19 and Number of Channels = 2
    ...
    % Collect the func handles for each feature func
    featHandles={   @(A)sum(abs(A));   
                    @(A)mad(A);   
                    @(A)var(A);   
                    @zeroCrossing;
                    @wilsonAmp;     
...
%     A_fmedian=fmedian(A);
%     A_fmean=fmean(A);
%     A_ar7=arN(A);                         %7th order AR coeff
% 
...
%      A_feature=[A_immg A_mad A_var A_zc A_wilson A_rms A_wlen A_fmedian A_fmean A_ar7];
...
%             A_ar7_coeff(1,:,i)=arN.a(2:8);
    ...
    %This function finds the median frequency of the given MMG sample
        ...
        for i=1:nCh
            ...
            totalPower=avgpower(Hpsd,[0,Fs/2-1]);    j=0;
