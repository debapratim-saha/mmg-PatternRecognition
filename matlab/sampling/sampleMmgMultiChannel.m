...
sampleWindowLen = 0.6;                              %Length of time for each sample (in seconds)
...
unifiedGlobEventPointer=zeros(max(m),2);
for i=1:max(m)
    if abs(diff(globEventPointer(i,1,:)))<sampleSize     
        ...
        if maxAmpIndex ~= minTimeIndex  && globEventPointer(i,2,minTimeIndex)<0.2*maxAmp              %Means the channel with max amplitude is not where event starts first, so check if the amp at the point of event start is atleast greater than 20% of maxAmp
            unifiedGlobEventPointer(i,:)  =   globEventPointer(i,1:2,maxAmpIndex);
        else
...
for i=1:max(m)-1   %for sample#26 and U1S3,U4S4 - have to delete this "-1" TODO: find reason
    ...
    eventSample=timeSeriesData(eventStart:eventEnd,:);
